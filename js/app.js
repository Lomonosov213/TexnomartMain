...
// работа главного слайдера
//------Начал работу отсюда и вниз. Поменял имена классов
function sliders_bild_callback(params) { }
...
if (document.querySelector('.mainslider')) {
	let mainslider = new Swiper('.mainslider__body', {
		...
		// Dotts
		pagination: {
			el: '.mainslider__dots',//куда отправляю точки
			clickable: true,
		},
		// Arrows
		navigation: {
	...
	//Это чтоб подтягивать уменьшеные картинки в пагинацию
/*
...
// Скрипт фильтра =======================
$('.catalog__view').click(function (event) {
   var i = $(this).data('filter');
   if (i == 1) {
      $('.goods__wrapp').show();
   } else {
      $('.goods__wrapp').hide();
      $('.goods__wrapp.f_' + i).show();
   }
   $('.catalog__view').removeClass('_active');
